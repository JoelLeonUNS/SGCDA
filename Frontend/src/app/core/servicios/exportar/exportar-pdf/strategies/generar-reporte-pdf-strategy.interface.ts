...
export class GenerarReportePdfStrategy implements GenerarPdfStrategy {
  ...
  generarPdf(parametros: ParametrosPdf): jsPDF {
    ...
    if (parametros.datosTabla && parametros.columnasTabla) {
      this.agregarTabla(parametros.datosTabla, parametros.montoTotal!, parametros.columnasTabla);
    }
  ...
  private agregarTabla(datosTabla: any[], montoTotal:number, columnasTabla: ColumnaTabla[]) {
    ...
    // Agregar total
    let footer: any[] = new Array(columnasTabla?.length).fill('');
    footer[0] = 'Total:';
    footer[footer.length - 1] = this.transPipe(montoTotal, { nombre: 'moneda', datos: [] });
  ...
  private agregarTotal(montoTotal: any) {
    autoTable(this.doc, {
      headStyles: {
        fillColor: [26, 82, 118],
        textColor: [255, 255, 255],
